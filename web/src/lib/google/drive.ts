...
export async function getOAuthClient(redirectUri?: string) {
  ...
  if (!data?.google_oauth_client_id || !data?.google_oauth_client_secret_encrypted) {
...
async function getDriveClient() {
  ...
  if (!data?.google_oauth_refresh_token_encrypted) {
    throw new Error('Google アカウントが未認可です。設定画面で認可してください。')
  }
...
export async function getDriveSettings(): Promise<{
  ...
  const { data } = await admin
    .from('system_settings')
    .select('google_drive_template_folder_id, google_drive_work_folder_id, google_oauth_client_id, google_oauth_refresh_token_encrypted')
    .single()
...
export async function listTemplateFolders(): Promise<DriveFolder[]> {
  ...
  const folderCheck = await drive.files.get({
    ...
    throw new Error(`テンプレートフォルダへのアクセスに失敗しました: ${err.message}`)
  })
...
async function copyFolderRecursive(
  ...
  console.log('[drive] items in', folderName, ':', items.map(i => i.name))

  await Promise.all(
    items.map(async (item) => {
      try {
        if (item.mimeType === 'application/vnd.google-apps.folder') {
          await copyFolderRecursive(drive, item.id!, newFolderId, item.name!, projectName, docs, replacements)
        } else {
          const newFileName = `${projectName}_${item.name!}`
          ...
          if (
            ...
            try {
              ...
              console.warn('[docs] placeholder replacement failed:', newFileName, err)
            }
          }
        }
      } catch (err) {
...
export async function copyTemplateFolder(
  ...
  const drive = await getDriveClient()
  const docs = (replacements && Object.keys(replacements).length > 0)
    ? await getDocsClient()
    : undefined
  ...
  const newFolderName = `${yyyy}${mm}${dd}_${projectName}_${sourceFolderName}`

  console.log('[drive] copying:', sourceFolderName, '→', newFolderName)
  const newFolderId = await copyFolderRecursive(drive, sourceFolderId, workFolderId, newFolderName, projectName, docs, replacements)
